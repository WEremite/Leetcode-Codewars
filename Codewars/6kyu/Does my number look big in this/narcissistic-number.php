...
{
    $splittedNumber = str_split($value);
    $sumOfPower = array_sum(
        array_map(fn ($digit) => $digit ** count($splittedNumber),
        $splittedNumber));

    return $sumOfPower === $value;
}

var_dump(narcissistic(7)); // true
var_dump(narcissistic(371)); // true
var_dump(narcissistic(153)); // true
var_dump(narcissistic(1652)); // false
