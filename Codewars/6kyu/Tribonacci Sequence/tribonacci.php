...
but summing the last 3 (instead of 2) numbers of the sequence to generate the next. And,
...
recommend to any math enthusiast and for showing me this mathematical curiosity too with his usual contagious
passion :)]
...
function tribonacci(array $signature, int $n): array
{
    if ($n < count($signature)) {
        ...
        return array_chunk($signature, $n)[0];
    }

    $additionalSequenceLength = $n - count($signature);

    for ($i = 0; $i < $additionalSequenceLength; $i++) {
        $sumOfElem = $signature[$i] + $signature[$i + 1] + $signature[$i + 2];
        $signature[] = $sumOfElem;
    }
...
print_r(tribonacci([1,1,1],10)); // [1,1,1,3,5,9,17,31,57,105]
print_r("\n");
print_r(tribonacci([0,0,1],10)); // [0,0,1,1,2,4,7,13,24,44]
print_r("\n");
print_r(tribonacci([0,1,1],10)); // [0,1,1,2,4,7,13,24,44,81]
print_r("\n");
print_r(tribonacci([1,1,1],1));
print_r("\n");
print_r(tribonacci([300,200,100],0));
print_r("\n");